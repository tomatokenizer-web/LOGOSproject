...
domainBonus = 0.1 if 도메인 관련 접사 존재, else 0
```
...
inflectionPenalty = 0.1 if 굴절 있음, else 0
```
...
### 굴절 접미사 감지 (lines 491-519)

```typescript
function detectInflectionSuffix(word: string) {
  if (word.endsWith('ing')) return { suffix: 'ing', meaning: '진행형' };
  if (word.endsWith('ed'))  return { suffix: 'ed', meaning: '과거시제' };
  if (word.endsWith('ies')) return { suffix: 'ies', meaning: '복수 (y→ies)' };
  if (word.endsWith('es'))  return { suffix: 'es', meaning: '복수' };
  if (word.endsWith('s'))   return { suffix: 's', meaning: '복수/3인칭' };
  if (word.endsWith('est')) return { suffix: 'est', meaning: '최상급' };
  return null;
}
```
...
function computeMorphologicalScore(
  ...
  // 1. Family size 정규화 (log 척도, 최대 50)
...
- `contraindication`: contra- (0.6) + in- (0.7) + -tion (0.9) → complex → 낮음
...
| `analyzeMorphology` | 1031-1124 | O(p×m+s×m) | 완전 형태론 분석 |
| `toMorphologicalVector` | 1254-1278 | O(1) | 벡터 생성 |
| `buildMorphologicalFamily` | 639-725 | O(w) | 형태론적 가족 |
| `computeMorphologicalScore` | 870-916 | O(w) | M 점수 계산 |
...
morphology.ts (독립, 외부 의존성 없음)
  ...
  │
  └──> 서비스:
       ├── task-generation.service (형태론 과제)
       ├── vocabulary-extraction (형태론 정보 추출)
